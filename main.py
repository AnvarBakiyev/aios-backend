...
def setup_tables():
    ...
    try:
        ...
        cur.execute('CREATE TABLE IF NOT EXISTS tasks ('
            ...
            'source VARCHAR(255), status VARCHAR(50) DEFAULT \'pending\',' 
            'created_at TIMESTAMP DEFAULT NOW(), completed_at TIMESTAMP)')
        cur.execute('CREATE TABLE IF NOT EXISTS task_executions ('
...
def dashboard():
    ...
    kpi.update(cur.fetchone())
    cur.execute(
        'SELECT c.id, c.name, c.industry, c.city, c.status, c.contract_value,'
        'c.roi_realized, COUNT(DISTINCT t.id) tasks_total,'
...
def create_client(payload: dict):
    ...
    cur.execute(
        'INSERT INTO clients(id,name,industry,city,employees_total,contract_value,'
        "status,created_at,roi_realized,tasks_completed) VALUES(%s,%s,%s,%s,%s,%s,'setup',NOW(),0,0)",
...
def create_task(payload: dict):
    ...
    cur.execute(
        ...
        (tid, payload['client_id'], payload['employee_id'], payload['title'],
         ...
         payload.get('priority','normal'), payload.get('roi_estimate',''),
...
def complete_task(task_id: str, payload: dict):
    ...
    eid = str(uuid.uuid4())
    roi = float(payload.get('roi_realized', 0))
    cur.execute(
    ...
    cur.execute(
        'UPDATE clients SET tasks_completed=tasks_completed+1,'
...
def register_agent(payload: dict):
    ...
    cur.execute(
        'INSERT INTO agents(id,client_id,name,description,category,status,'
        "runs_total,success_rate,created_at) VALUES(%s,%s,%s,%s,%s,'active',0,100.0,NOW())",
...
def ping_agent(agent_id: str, payload: dict = {}):
    ...
    cur.execute(
        'UPDATE agents SET runs_total=runs_total+1, last_run=NOW(),'
        'success_rate=(success_rate*runs_total+%s)/(runs_total+1) WHERE id=%s',
        (s, agent_id))
    c.commit(); cur.close(); c.close()
    return {'status': 'ok'}